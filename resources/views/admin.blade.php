...
@section('contenido')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mt-10 mb-6 gap-4">
        ...
        <div class="flex items-center gap-3 flex-wrap">
            <a href="{{ route('admin.categorias.edit') }}" class="bg-white text-gray-700 border border-gray-300 px-5 py-3 rounded-[10px] font-bold text-[0.9rem] hover:bg-gray-50 hover:text-[#f89a20] hover:border-[#f89a20]/50 hover:-translate-y-0.5 hover:shadow-md transition duration-200 inline-flex items-center gap-2 no-underline shadow-sm">
                <i class="fa-solid fa-folder text-[#f89a20]"></i> Gestión de Categorías
            </a>
            <a href="{{ route('admin.distritos.index') }}" class="bg-white text-gray-700 border border-gray-300 px-5 py-3 rounded-[10px] font-bold text-[0.9rem] hover:bg-gray-50 hover:text-[#f89a20] hover:border-[#f89a20]/50 hover:-translate-y-0.5 hover:shadow-md transition duration-200 inline-flex items-center gap-2 no-underline shadow-sm">
                <i class="fa-solid fa-map-location-dot text-[#f89a20]"></i> Gestionar Distritos
            </a>
            <a href="{{ route('admin.pedidos.index') }}" class="bg-white text-gray-700 border border-gray-300 px-5 py-3 rounded-[10px] font-bold text-[0.9rem] hover:bg-gray-50 hover:text-[#f89a20] hover:border-[#f89a20]/50 hover:-translate-y-0.5 hover:shadow-md transition duration-200 inline-flex items-center gap-2 no-underline shadow-sm">
                <i class="fa-solid fa-truck-fast text-[#f89a20]"></i> Gestionar Envíos
            </a>
            <a href="{{ route('admin.productos.create') }}" class="bg-[#f89a20] text-white px-5 py-3 rounded-[10px] font-bold text-[0.9rem] hover:bg-[#e0891b] hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#f89a20]/35 transition duration-200 inline-flex items-center gap-2 no-underline">
    ...
    <div class="bg-white border border-[#e5e7eb] rounded-[24px] overflow-x-auto shadow-[0_10px_30px_rgba(0,0,0,0.02)] mb-20">
            ...
            <tbody>
                @forelse($productos as $producto)
                    <tr class="border-b border-[#e5e7eb] last:border-none hover:bg-gray-50/50 transition">
                        ...
                        <td class="p-4 px-6">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold {{ $producto->activo ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ $producto->activo ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
                        ...
                        <td class="p-4 px-6 text-center">
                            <div class="flex gap-2 justify-center">
                                ...
                                <form action="{{ route('admin.productos.destroy', $producto->id_producto) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas ocultar este producto del catálogo?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-9 h-9 bg-[#fee2e2] text-rose-600 rounded-lg flex items-center justify-center hover:bg-[#fca5a5] transition duration-150 cursor-pointer border-none" title="Desactivar producto">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center p-8 text-[#6b7280]">
